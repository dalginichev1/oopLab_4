...
Trapez<T>::Trapez(Point<T> _p1, Point<T> _p2, Point<T> _p3, Point<T> _p4)
    : p1(std::make_unique<Point<T>>(_p1)), p2(std::make_unique<Point<T>>(_p2)), p3(std::make_unique<Point<T>>(_p3)), p4(std::make_unique<Point<T>>(_p4)) {}
...
{
    return 0.5 * (p1->x()*(p2->y() - p4->y()) + p2->x()*(p3->y() - p1->y()) + p3->x()*(p4->y() - p2->y()) + p4->x()*(p1->y() - p3->y()));
}
...
bool Trapez<T>::operator==(const Figure<T>(& other)) const
{
    ...
    if (trapez != nullptr)
    {
        return (trapez->area() == area()) && (trapez->center() == center());
    }
